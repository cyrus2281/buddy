...
const api: BuddyApi = {
  ...
  activate: () => ipcRenderer.invoke(CH.activate),
  getInference: () => ipcRenderer.invoke(CH.getInference),

  getNotes: (type, limit) => ipcRenderer.invoke(CH.getNotes, type, limit),
  searchNotes: (query, type) => ipcRenderer.invoke(CH.searchNotes, query, type),
  getNoteDetail: (id) => ipcRenderer.invoke(CH.getNoteDetail, id),
  updateNote: (id, patch) => ipcRenderer.invoke(CH.updateNote, id, patch),
  deleteNote: (id) => ipcRenderer.invoke(CH.deleteNote, id),
  setTaskStatus: (id, status) => ipcRenderer.invoke(CH.setTaskStatus, id, status),
  setTaskScope: (id, scope) => ipcRenderer.invoke(CH.setTaskScope, id, scope),
  getNotesStats: () => ipcRenderer.invoke(CH.getNotesStats),
  getTodayRecap: () => ipcRenderer.invoke(CH.getTodayRecap),
  getOpenTasks: () => ipcRenderer.invoke(CH.getOpenTasks),
  getObservations: (limit) => ipcRenderer.invoke(CH.getObservations, limit),
  getSpend: () => ipcRenderer.invoke(CH.getSpend),
  resetSpend: () => ipcRenderer.invoke(CH.resetSpend),
  observeNow: () => ipcRenderer.invoke(CH.observeNow),
  ...
  onInference: (fn) => subscribe(CH.onInference, fn),
